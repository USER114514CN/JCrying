...
public class DesEdeEncryptor extends GeneralEncryptor {
    private static final String TRANSFORM = "DESede/CBC/PKCS5Padding";
    private static final int IV_BYTE_LEN = 8;
    private static final int FIX_KEY_BYTE_LEN = 24;
    ...
    public byte[] encrypt(byte[] data, byte[] key) {
        ...
        if (key.length != FIX_KEY_BYTE_LEN) {
            throw new IllegalArgumentException("3DES(DESede)密钥必须为24字节");
        }
        ...
        try {
            ...
            throw new IllegalStateException("3DES加密发生异常", e);
        }
    }
    ...
    public byte[] decrypt(byte[] data, byte[] key) throws IllegalDataException {
        ...
        if (key.length != FIX_KEY_BYTE_LEN) {
            throw new IllegalDataException("3DES(DESede)密钥必须为24字节");
        }
        ...
        try {
            ...
            throw new IllegalDataException("3DES解密失败：密钥错误或数据损坏", e);
        }
    }
}
